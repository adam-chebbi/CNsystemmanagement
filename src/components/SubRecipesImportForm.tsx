...
import {
  SUBRECIPE_IMPORT_COLUMNS,
  MAX_IMPORT_FILE_SIZE_BYTES,
  ImportedSubRecipeRowDraft,
  ImportFileError,
  parseSubRecipeImportFile,
  buildSubRecipesFromImportRows,
  downloadSubRecipeImportTemplateCsv,
} from '../data/importSubRecipesParser';
import { collapseValidRows, toggleInSet } from '../data/importReviewUtils';

interface SubRecipesImportFormProps {
...
export const SubRecipesImportForm: React.FC<SubRecipesImportFormProps> = ({ subRecipes, ingredients, units, onImportSubRecipes, onClose }) => {
  ...
  const [rows, setRows] = useState<ImportedSubRecipeRowDraft[]>([]);
  ...
  const describeLine = (line: ImportedSubRecipeRowDraft['ingredients'][number]): string => {
    if (line.kind === 'subrecipe') {
      const sr = subRecipes.find((s) => s.id === line.subRecipeId);
      return `Sous-recette « ${sr?.name ?? '?'} » — ${line.quantity} ${line.unit}`;
    }
    const ing = ingredients.find((i) => i.id === line.ingredientId);
    return `${ing?.name ?? '?'} — ${line.quantity} ${line.unit}`;
  };
  ...
  const processFile = async (candidate: File) => {
    ...
    setIsParsing(true);
    try {
      const result = await parseSubRecipeImportFile(candidate, subRecipes, ingredients, units);
      setRows(result.rows);
  ...
  const handleConfirm = async () => {
    ...
    setSaveError(null);
    try {
      const toCreate = buildSubRecipesFromImportRows(rows);
      await onImportSubRecipes(toCreate);
  ...
  if (step === 'success') {
    return (
      <div className="p-8 sm:p-12 rounded-2xl bg-white dark:bg-[#151D2A] border border-gray-100 dark:border-gray-800 shadow-2xs flex flex-col items-center text-center gap-3">
        ...
        <h2 className="text-lg font-bold text-gray-900 dark:text-white">Sous-recettes créées avec succès</h2>
        <p className="text-xs text-gray-500 dark:text-gray-400 max-w-md">
          {savedCount} nouvelle(s) sous-recette(s) {savedCount > 1 ? 'ont été ajoutées' : 'a été ajoutée'} au catalogue.
        </p>
        <div className="flex items-center gap-2 pt-1">
          ...
          <button onClick={onClose} className={primaryButtonClass}>
            <Beaker size={14} />
            <span>Voir les sous-recettes</span>
          </button>
        </div>
      </div>
    );
      ...
      <div className="space-y-4">
        <div className="p-4 rounded-2xl bg-purple-50 dark:bg-purple-950/20 border border-purple-200 dark:border-purple-800/50 flex items-start gap-3">
          ...
          <div className="text-xs text-purple-700 dark:text-purple-300">
            <p className="font-semibold mb-1">Créer de nouvelles sous-recettes</p>
            <p>
              Chaque ligne crée une nouvelle sous-recette. Un nom déjà utilisé sera rejeté — modifiez-le directement
              dans la liste plutôt que de le réimporter. Une sous-recette référençant une autre sous-recette doit
              référencer une sous-recette qui existe déjà (créée à la main ou importée dans un fichier précédent).
            </p>
          </div>
        </div>
        ...
        <div className="p-4 rounded-2xl bg-white dark:bg-[#151D2A] border border-gray-100 dark:border-gray-800 shadow-2xs space-y-2.5">
          <div className="flex items-center justify-between">
            ...
            <button onClick={downloadSubRecipeImportTemplateCsv} className={secondaryButtonClass}>
              <Download size={13} />
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-xs">
              ...
              <tbody>
                {SUBRECIPE_IMPORT_COLUMNS.map((c) => (
    ...
    <div className="space-y-4">
      <div className="p-4 rounded-2xl bg-purple-50 dark:bg-purple-950/30 border border-purple-200 dark:border-purple-800/60 flex items-start gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-purple-700/80 dark:text-purple-400/80">
            Aucune sous-recette n'est encore créée. Contrôlez les lignes ci-dessous, puis confirmez.
          </p>
        </div>
      </div>
      ...
      <div className="space-y-2.5 max-h-[700px] overflow-y-auto custom-scrollbar pr-0.5">
        {rows.map((row) => {
          ...
          return (
            <div
              key={row.id}
              className={`rounded-xl border p-3 space-y-2 ${hasError ? 'border-red-300 dark:border-red-800/70 bg-red-50/40 dark:bg-red-950/10' : 'border-gray-100 dark:border-gray-800 bg-white dark:bg-[#151D2A]'}`}
            >
              <div className="flex items-center gap-2">
                  ...
                  <div className="min-w-0">
                    ...
                    {isCollapsed && (
                      <p className="text-[11px] text-gray-400 truncate">
                        {row.yieldQuantity} {row.yieldUnit} • {row.ingredients.length} composant(s)
                      </p>
                    )}
                  </div>
                </button>
      ...
      <div className="p-4 rounded-2xl bg-white dark:bg-[#151D2A] border border-gray-100 dark:border-gray-800 shadow-2xs flex flex-col sm:flex-row items-center justify-between gap-3 sticky bottom-4">
        <div className="flex items-center gap-2 text-xs text-gray-500 dark:text-gray-400">
          ...
          <span><strong className="text-gray-800 dark:text-gray-200">{rows.length}</strong> sous-recette(s) prête(s)</span>
        </div>
        <button onClick={handleConfirm} disabled={!canConfirm || isSaving} className={`${primaryButtonClass} w-full sm:w-auto`}>
          ...
          <span>{isSaving ? 'Création…' : `Confirmer et créer ${rows.length} sous-recette(s)`}</span>
        </button>
      </div>
    </div>
  );
