...
export const SuppliersImportPage: React.FC<SuppliersImportPageProps> = ({
  ...
  const buildRowSummaryLine = (row: ImportedSupplierDraft): string => {
    const parts: string[] = [];
    if (row.draft.phone.trim()) parts.push(row.draft.phone);
    if (row.draft.email.trim()) parts.push(row.draft.email);
    if (row.draft.mainContact.trim()) parts.push(row.draft.mainContact);
    return `${row.draft.name || 'Sans nom'}${parts.length ? ` — ${parts.join(' • ')}` : ''}`;
  };
